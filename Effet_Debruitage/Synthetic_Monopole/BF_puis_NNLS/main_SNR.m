...
%%% Global constants
%%%-----------------------------------------------------------------------------
methods={'RPCA','DR','DR_Quentin','none'};%{'none','DRec','RPCA','DR','DR_Quentin'};%{'none','DRec','RPCA','PFA','DR'}; %denoising methods to apply
fftsign=+1;
...
for i=1:length(methods)
    file_name=['out_data/' methods{i} '_SNR.h5'];
    h5create(file_name,'/SNR', [length(SNR) 1]);
    h5create(file_name,'/Amplitude_Source',[length(SNR) Nsrc]);
end
...
%%
        ...
        h5write(['out_data/' methods{i} '_SNR.h5'],'/SNR',SNR(j),[j 1],[1 1]);
        h5write(['out_data/' methods{i} '_SNR.h5'],'/Amplitude_Source',amp_src,[j 1],[1 Nsrc]);
...
legend('show');
for i=1:length(methods)
    amp=h5read(['out_data/' methods{i} '_SNR.h5'],'/Amplitude_Source');
    plot(SNR,10*log10(sum(amp(:,:),2)./Nsrc),'o-','DisplayName',methods{i})
end
