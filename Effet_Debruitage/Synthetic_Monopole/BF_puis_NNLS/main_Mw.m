...
%%% Global constants
%%%-----------------------------------------------------------------------------
methods={'RPCA','DR','DR_Quentin','none'};%{'none','DRec','RPCA','PFA','DR'}; %denoising methods to apply
fftsign=+1;
...
SNR=10;%noise level added
Mw=unique(round(logspace(log10(93),log10(10^4),10))); %number of snapshots
c=340;
...
G_map_mic = GreenFreeField(r_map, r_mic , freq, -fftsign, 340); %monopole in free field
%%

%Delete former data files and create new file
for i=1:length(methods)
    file_name=['out_data/' methods{i} '_Mw.h5'];
    delete(file_name);
    h5create(file_name,'/Mw', [length(Mw) 1]);
    h5create(file_name,'/Amplitude_Source',[length(Mw) Nsrc]);
end
...
for j=1:length(Mw) %vary the number of snapshots
    disp(['Mw : ' num2str(Mw(j))]);
    
	%%%-----------------------------------------------------------------------------
	%%% Generate CSM
	%%%-----------------------------------------------------------------------------
	[Sq Sy Sp Sn ] = generate_Spp_signal(freq, Nsrc , rho , SNR , Mw(j),SNR , x_src , z_src,r_mic); %no extra-noise
	%%
...
%%
        ...
        h5write(['out_data/' methods{i} '_Mw.h5'],'/Mw',Mw(j),[j 1],[1 1]);
        h5write(['out_data/' methods{i} '_Mw.h5'],'/Amplitude_Source',amp_src,[j 1],[1 Nsrc]);
...
for i=1:length(methods)
	amp=h5read(['out_data/' methods{i} '_Mw.h5'],'/Amplitude_Source');
	semilogx(Mw,10*log10(sum(amp(:,:),2)./Nsrc),'o-','DisplayName',methods{i})
	hold on
end
plot(Mw,zeros(1,length(Mw)),':')
xlabel('Mw')
