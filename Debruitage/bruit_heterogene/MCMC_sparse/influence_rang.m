...
for i=1:length(Nsrc)
	...
	%%% Generate data
	[Sq Sy Sp Sn] = generate_Spp_signal_hetero(freq, Nsrc(i) , rho , SNR , Mw,SNR-10);
    ...
    alpha2_mean = abs(linspace(max(real(eig(Sy))),min(real(eig(Sy))),M))./max(real(eig(Sy)));
...
save('MCMC_Nsrc','err_mcmc','SNR','Nsrc','d_mcmc', 'd_ref','alpha_save');
%figure
