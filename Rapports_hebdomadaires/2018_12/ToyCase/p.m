...
for i = 2:Nrun
    for p=1+indx:P
        ...
        % =======================
        Omega_inv = kron(Dq*conj(squeeze(Sc(i,:,:,p)))*Dq,sparse(D)) + K*speye(MK);
        ...
        for k=1:K
            ...
            b_r = real(2*K* conj(Lambda(i,:,k,p))*Lambda(i,:,k,p).');
            p_r = Isnap(p) - M -0.5;
           
            r = gigrnd( p_r, a_r, b_r,1);
            
            
            s(i,k) = sqrt(r)*exp(1i*2*pi*rand); %(-1)^(round(rand)) * sqrt(r);
            Lambda(i,:,k,p) = Lambda(i,:,k,p)./s(i,k);
        end
        Sc(i,:,:,p) = squeeze(Sc(i,:,:,p)).*(s(i,:)'*s(i,:));
        %}
        %update Cov and Wc
        %{
   ...
   %}
        ...
        for k=1:K           
            ...
            elseif strcmp(opt.marg,'on') %avec marginalisation
                Bini = squeeze(Lambda(i,:,:,p))*Dq*diag(gamma2(i,:,p))*Dq*squeeze(Lambda(i,:,:,p))' + beta2(i-1,p)*diag(sig2(i-1,:));
                Bini=(Bini+Bini')/2;
