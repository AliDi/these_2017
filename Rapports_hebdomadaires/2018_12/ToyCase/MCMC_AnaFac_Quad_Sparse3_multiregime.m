...
if nargin > 7
    ...
    gamma2(1,:,1+indx:end) = Ini.gamma2*ones(K,1); %(NrunxP)
else
...
Er = zeros(M,1);
if indx
    Er(:,1)=diag(Sy(:,:,1)); %if noise ref is used, constant value given by noise
end
%h=waitbar(0,'MCMC...');
for i = 2:Nrun
    %waitbar(i/Nrun,h,'MCMC...')
    ...
    for p=1+indx:P
    	...
    	Da = diag(alpha(i-1,:,p));
        Cov_inv = Da*LambdaD*squeeze(Lambda(i-1,:,:,p))*Da + diag(1./gamma2(i-1,:,p));
