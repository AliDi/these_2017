...
\begin{frame}{\insertsectionhead}
	...
	\begin{itemize}
			...
			\node(Ldescr) [below left=0.2cm and 0.8cm of L,align=center,font=\footnotesize] {
	    		...
	    		\draw[->,>=stealth] (cdescr) to ($(c.south)+(4pt,2pt)$);
		...
		\begin{itemize}
		        ...
		        $\hookrightarrow$ low-rank acoustic CSM
...
\begin{frame}{\insertsectionhead}
	\begin{tikzpicture}[remember picture, overlay]
		...
		\node at ($(current page)-(2cm,1cm)$) {\parbox{8cm}{
		\begin{itemize}
	        		\item Cruise flight condition: Mach 0.85
	        		\item 5 different engine speed \\+ 1 background meas. (idle speed)
	        		\item 35 microphones flushmounted on the aft  \tikzmark{ext}\\ fuselage
	        		\item 9 microphones + 6 accelerometers in the cabin\tikzmark{int}
	        		\item CSM:
	        		\begin{itemize}
		...
		\draw[->,>=stealth,source,line width=1pt] ($(int)+(0,1.5ex)$)  to ($(a)+(1.7cm,1.5cm)$);% [out=-45,in=135]
	\end{tikzpicture}
\end{frame}

\captionsetup[subfigure]{labelformat=empty}
\begin{frame}{\insertsectionhead}
Autospectra of the outer mic. in the MF frequency range\\[-2em]
...
\parbox{\textwidth}{
	\begin{figure}
		...
		\subfloat[\scriptsize \itshape After background subtraction ]{\includegraphics[trim = 1.3ex 1ex 0 0,clip,width=0.385\textwidth]{./airbus/as_bgsub.png}}}\\%Background subtraction
		...
		\subfloat[\scriptsize \itshape Reference-based denoising]{\includegraphics[trim = 2ex 1ex 0 0,clip,width=0.32\textwidth]{./airbus/as_ref.png}}\\% Referenced denoising
	\end{figure}
	}
	\begin{tikzpicture}[remember picture, overlay]
		\node[anchor=south west] (text) at ($(current page.south west)+(0.8cm,1cm)$) {
		\begin{minipage}[c][0.25\textwidth][l]{0.36\textwidth}
			\small
			\vfill
		...
		\uncover<2->{
			\node[draw, ellipse,minimum height=2.3cm, minimum width=1.5cm,anchor=west] (interf) at ($(current page.south west)+(5.7,2.8)$) {};
...
{\footnotesize Iterative Reweighted Least Squares, $p=0$ + bayesian regularization}\\[0.5ex]
...
\begin{minipage}{1.1\textwidth} 	
 		...
 		\begin{tikzpicture}[remember picture,overlay]
        	...
        	\end{tikzpicture}
 \end{minipage}~\\[0.5ex]
\textbf{\footnotesize Conventional Beamforming}
