...
\noindent $\bullet$ \textbf{Mesures multivoies} en présence de \textbf{bruit} : veine d'essai, extérieur,...\\
...
\noindent $\bullet$  \textbf{Contexte  industriel} : étude des sources de bruit d'un avion de ligne (moteur, profil)\\
...
%mise à zéro ou soustraction : pourquoi ?\\
 ...
 \noindent Séparer la contribution des sources acoustique du bruit de couche limite turbulente.\\
 }
...
\begin{minipage}{\textwidth-3.1cm}
 ...
 -bruit faiblement corrélé : MI diagonale\\
 -signal corrélé, faible nombre de sources : MI à rang réduit\\[-2ex]
\end{minipage}

 }
 
 
 \headerbox{Méthode proposée}{name=methode,below=idee,column=0,span=6}{
 \centering \vspace{-1em} Faire une décomposition matricielle par de l'optimisation bayésienne :\\[1.5em]
...
\hfill
 \begin{minipage}[t]{0.23\textwidth}
		...
		\includegraphics[width=0.4\textwidth]{signal.png}  \raisebox{0.15\textwidth}{\large $+$} \includegraphics[width=0.4\textwidth]{bruit.png}
\end{minipage}
...
\end{minipage}
 ...
 \headerbox{Outils}{name=gibbs,below=methode,column=0,span=3}{
 \noindent Inférence bayésienne : échantillonneur de Gibbs\\
 modèle a variables latentes\\
 Gibbs : Permet d'approximer la distribution jointe inconnue $p(\theta_1,\theta_2,\dots|\bm{S}_{pp})$ à partir des distributions conditionnelles connues $p(\theta_1|\bm{S}_{pp},\theta_2,\dots)$. C'est une méthodes de Monte-Carlo par chaînes de Markov : une marche aléatoire biaisée.
 }

 
 \headerbox{Utilisation du bruit de fond et des mesures multiples}{name=bf,span=3,column=3 , below=methode}{
 Bruit commun à tous les régimes à un facteur près :
For i
  Pour chaque régime moteur
Tirer $L^i$ dans $p(L|S_y^p, \text{tous les paramètres}^{i-1})$ 
 Tirer $S_c^i$ dans $p(S_c|S_y^p, \text{tous les paramètres}^{i-1})$
...
$\bullet$ Étude du bruit de jet supersonique, not. des cellules de chocs (monopoles corrélés)\\
$\bullet$ Méthode d'imagerie : IRLS régularisation bayésienne p=0
\end{minipage}
\hfill
...
\centering
 ...
 \headerbox{Analyse}{name=analyse,column=0, span=3,below=imagerie}{
 \begin{minipage}{0.45\textwidth}
 \centering
  \resizebox{0.5cm}{!}{\circled{\textbf{+}}}\\[1ex]
  Intervalle de crédibilité\\
  Réduction de dimension\\
  Sélection automatique de l'ordre : pas de paramètre à régler\\
  Modèle flexible  
\end{minipage}\hfill
\begin{minipage}{0.45\textwidth}
\centering
   \resizebox{0.5cm}{!}{\circled{$\boldmath{-}$}}\\[1ex]
   Sensibilité aux priors selon conditionnement\\
   Coût de calcul élevé
\end{minipage}
 }
 
 \headerbox{Perspectives}{name=perspective,column=3, span=3,below=imagerie}{
 Échantillonneur plus robuste et plus rapide\\
 Modèle à adapter, not. pour la corrélation du bruit
 }
