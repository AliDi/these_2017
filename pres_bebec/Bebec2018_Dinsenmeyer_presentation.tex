...
\begin{frame}[t]{\insertsectionhead~--~CSM properties}
	...
	\only<2->{
	$$\Bigg\langle \underbracket[0.5pt]{~\bm{p}~}_{\text{measured spectra}} =\underbracket[0.5pt]{~\textcolor{green}{\bm{a}}~}_{\text{source spectrum}} + \underbracket[0.5pt]{~\textcolor{red}{\bm{n}}~}_{\text{Gaussian noise}}  \Bigg\rangle_{\text{\normalsize $ N_s$ snapshots}}$$
	}
	\end{overlayarea}
...
\begin{frame}{\insertsectionhead~--~CSM properties}
	...
	\textbf{For $N_s \rightarrow \infty$}\\
	\pause
	\begin{itemize}
...
%	\end{mdframed}
	...
	\begin{block}{Select Convex Optimization (DRec) for further comparison}
		...
		\ding{51} Better performance		 \hfill\parbox{0.5\linewidth}{\ding{55} Denoises only diagonal}\\
	\end{block}
...
\begin{frame}
Diapo d'intro aux méthodes bayésiennes ?
\end{frame}
	...
	\only<2>{
		\vspace{1cm}
		 ...
		 \setlength{\itemindent}{0.5cm}
       		 	\item[$\hookrightarrow$] Error linearly decreases with increasing SNR\\[5pt]
       		 	\item[$\hookrightarrow$] Error linearly decreases with logarithmically increasing $N_s$\\[5pt]
       			\item[$\hookrightarrow$] For $N_{src} \geq 0.75 M$: denoising problem becomes poorly conditioned       \\[5pt]		 
		\end{itemize}
	}	
